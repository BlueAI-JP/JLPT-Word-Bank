...
async def get_all_users_admin() -> list[dict]:
    ...
    Each one-to-many table is pre-aggregated in a derived subquery before
    joining, so the join is always 1:1 — no Cartesian explosion risk.
    mastered_words has no 'id' column; COUNT(*) is used inside its subquery.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("""
            SELECT
                ...
                COALESCE(mw.mastered_count, 0) AS mastered_count,
                qs.best_score                  AS best_quiz_score,
            ...
            LEFT JOIN (
                SELECT user_id, COUNT(*) AS quiz_count, MAX(score) AS best_score
                FROM quiz_sessions GROUP BY user_id
            ) qs ON qs.user_id = u.id
            LEFT JOIN (
                SELECT user_id, COUNT(*) AS mastered_count
                FROM mastered_words GROUP BY user_id
